...
import com.gambling.websites.flashscore.domain.TennisMatchScore
import com.gambling.websites.flashscore.domain.Score
import com.gambling.websites.flashscore.domain.SetScore
import scala.Some
import com.gambling.websites.flashscore.domain.Player
...
class TennisPage(implicit driver: WebDriver) {
  ...
  def getMatches: Seq[TennisMatchScore] = {
    ...
    var result: List[TennisMatchScore] = List[TennisMatchScore]()
    while (iterator.hasNext) {
      ...
      if (firstRow.getElementsByClass("blank-line").size() == 0) {
        if (iterator.hasNext) {
          ...
          result = result :+ new TennisMatchScore(
...
object TennisPage {
  ...
  object Parser {
    ...
    private val NAME_PATTERN_FALLBACK = Pattern.compile("(.*)\\.(.*)\\..*")

    def parsePlayerName(el: Element): Seq[Player] = {
      el.text().split('/').map(name => {

        val matcher: Matcher = NAME_PATTERN.matcher(name)
        if (matcher.matches()) {
          new Player(matcher.group(1), matcher.group(2))
        } else {
          val fallbackMatcher: Matcher = NAME_PATTERN_FALLBACK.matcher(name)
          if (fallbackMatcher.matches()) {
            new Player(fallbackMatcher.group(1), fallbackMatcher.group(2))
          } else {
            throw new IllegalArgumentException(name + " is not matching name pattern")
          }
        }
      })
    }

    def parseMatchScore(home: Element, away: Element): Score = {
